...
class statical_test:
    ...
    def __init__(self, results, model,Data_train = None , X_train = None ,y_train = None, scoring = 'f1', SAVE_PREFIX=''):
   ...
   #Create dataframe for Posthocs
        ...
        plt.savefig(f"{self.SAVE_PREFIX}/Wilcoxon_heatmap.png", dpi = 600, bbox_inches="tight")
        plt.savefig(f"{self.SAVE_PREFIX}/Wilcoxon_heatmap.pdf", dpi = 600, format='pdf',transparent = False,bbox_inches="tight")
        plt.savefig(f"{self.SAVE_PREFIX}/Wilcoxon_heatmap.pvg", dpi = 600, format='svg',transparent = False,bbox_inches="tight")
        plt.savefig(f"{self.SAVE_PREFIX}/Wilcoxon_heatmap.eps", dpi = 600, format='eps',transparent = False,bbox_inches="tight")
        ...
        **kwargs) -> Union[SubplotBase, Tuple[SubplotBase, Colorbar]]:
        ...
        if flat:
          ...
          if not labels:
            hax.sex_xlabel("")
            hax.sex_ylabel("")
          hax.tick_params(axis='both',labelsize=10)
    ...
    def visualize(self, box_text_size = 'x-large'):
      #Vẽ biểu đồ boxplot thể hiện hiệu quả của từng model, từ đó chọn model phù hợp để tối ưu.
        ...
        for xtick in box_plot.get_xticks():
            ...
            horizontalalignment='center',size=box_text_size,color='w',weight='semibold', path_effects = [path_effects.withStroke(linewidth=3, foreground="k"), path_effects.Normal()])
        ...
        plt.savefig(f"{self.SAVE_PREFIX}/model_selection.png", dpi = 600, bbox_inches="tight")
        plt.savefig(f"{self.SAVE_PREFIX}/model_selection.pdf", dpi = 600, format='pdf',transparent = False,bbox_inches="tight")
        plt.savefig(f"{self.SAVE_PREFIX}/model_selection.svg", dpi = 600, format='svg',transparent = False,bbox_inches="tight")
        plt.savefig(f"{self.SAVE_PREFIX}/model_selection.eps", dpi = 600, format='eps',transparent = False,bbox_inches="tight")
