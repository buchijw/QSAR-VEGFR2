...
class statistic_test:
    ...
    def __init__(self, meta_data, save_data = False, scoring = 'f1', kind_analysis ='Meta',
                 posthoc_method ='Wilcoxon', refinement = False, SAVE_PREFIX=''):
        ...
        self.meta_folder = SAVE_PREFIX+"/Meta_folder"
    ...
    def posthoc(self):
       #Create dataframe for Posthocs
        ...
        plt.savefig(f"{self.meta_folder}/{self.posthoc_method}_{self.kind_analysis}_heatmap.png", dpi = 600, bbox_inches="tight")
        plt.savefig(f"{self.meta_folder}/{self.posthoc_method}_{self.kind_analysis}_heatmap.pdf", dpi = 600, format='pdf',transparent = False,bbox_inches="tight")
        plt.savefig(f"{self.meta_folder}/{self.posthoc_method}_{self.kind_analysis}_heatmap.svg", dpi = 600, format='svg',transparent = False,bbox_inches="tight")
        plt.savefig(f"{self.meta_folder}/{self.posthoc_method}_{self.kind_analysis}_heatmap.eps", dpi = 600, format='eps',transparent = False,bbox_inches="tight")
    ...
    def boxplot_visualize(self,data):
      #Vẽ biểu đồ boxplot thể hiện hiệu quả của từng model, từ đó chọn model phù hợp để tối ưu.
        ...
        for xtick in box_plot.get_xticks():
            box_plot.text(xtick,ser[xtick]+ vertical_offset,'%.3f'%(ser[xtick]), 
            horizontalalignment='center',color='w',weight='semibold',fontsize = 14, path_effects = [path_effects.withStroke(linewidth=2, foreground="k"), path_effects.Normal()])

        #box_plot.get_xticks(range(len(data.columns)))
        box_plot.set_xticklabels(data.columns, rotation='horizontal', fontsize = 14)
        box_plot.tick_params(axis='y', which='major', labelsize=14)
        box_plot.set_ylabel(f"{self.scoring}".upper(), fontsize=16, weight='semibold')
        plt.savefig(f"{self.meta_folder}/{self.kind_analysis}_compare_visualize.png", dpi = 600, bbox_inches="tight")
        plt.savefig(f"{self.meta_folder}/{self.kind_analysis}_compare_visualize.pdf", dpi = 600, format='pdf',transparent = False,bbox_inches="tight")
        plt.savefig(f"{self.meta_folder}/{self.kind_analysis}_compare_visualize.svg", dpi = 600, format='svg',transparent = False,bbox_inches="tight")
        plt.savefig(f"{self.meta_folder}/{self.kind_analysis}_compare_visualize.eps", dpi = 600, format='eps',transparent = False,bbox_inches="tight")
...
class statistic_data(statistic_test):
    ...
    def make_meta_data(self):
        ...
        for i in self.data_name:
            data = pd.read_csv(f"{self.data_dir}/{i}.csv").drop(['Unnamed: 0'], axis =1)
