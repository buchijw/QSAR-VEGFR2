...
class Featurize():
    ...
    def Apply(self):
        ...
        for i in rdk_type:
            ...
            df = pd.DataFrame(X)
            self.RDKF= pd.concat([self.RDKF, df], axis = 1).drop(["FPs", "Molecule", self.ID], axis =1)
        ...
        for i in ecfp_type:
            ...
            df = pd.DataFrame(X)
            self.ecfp= pd.concat([self.ecfp, df], axis = 1).drop(["FPs", "Molecule", self.ID], axis =1)
        ...
        df = pd.DataFrame(X)
        self.maccs= pd.concat([self.maccs, df], axis = 1).drop(["FPs", "Molecule", self.ID], axis =1)
        ...
        df = pd.DataFrame(X)
        self.avalon= pd.concat([self.avalon, df], axis = 1).drop(["FPs", "Molecule", self.ID], axis =1)
        ...
        df = pd.DataFrame(X)
        self.pubchem= pd.concat([self.pubchem, df], axis = 1).drop(["FPs", "Molecule", self.ID], axis =1)
        ...
        d = pd.DataFrame(X)
        self.map4= pd.concat([self.map4, d], axis = 1).drop(["map4", "Molecule", self.ID], axis =1)
        ...
        d = pd.DataFrame(X)
        self.secfp= pd.concat([self.secfp, d], axis = 1).drop(["secfp", "Molecule", self.ID], axis =1)
        ...
        d = pd.DataFrame(X)
        self.gobbi = pd.concat([self.gobbi, d], axis = 1).drop(["pharmgb", "Molecule", self.ID], axis =1)
        #self.gobbi.head(2)
        ...
        d = pd.DataFrame(X)
        self.cats2d = pd.concat([self.cats2d, d], axis = 1).drop(["cats2d", "Molecule", self.ID], axis =1)
        ...
        self.rdkdes = pd.concat([self.rdkdes, d], axis = 1).drop(["rdkdes", "Molecule", self.ID], axis =1)
        ...
        Chem.PandasTools.WriteSDF(self.mord, self.save_dir+'conf.sdf', molColName='Molecule')
        command = f"python -m mordred {self.save_dir+'conf.sdf'} -3 -o {self.save_dir+'Mordred.csv'}"
        subprocess.run(command, shell=True, check=True)
        
        self.mordred_df = pd.read_csv(self.save_dir+'Mordred.csv')
        self.mordred_df = self.mordred_df.drop('name', axis=1)
        self.mordred_df = pd.concat([self.mord[[self.activity_col,self.smile_col]], self.mordred_df], axis=1)
        self.mordred_df.to_csv(f"{self.save_dir}Mordred.csv", index=False)
        ...
        self.mol2vec = pd.concat([self.data[[self.activity_col,self.smile_col]], pd.DataFrame(X)], axis = 1)
